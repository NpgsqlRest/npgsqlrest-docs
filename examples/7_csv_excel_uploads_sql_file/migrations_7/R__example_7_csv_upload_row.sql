-- Row processor for CSV uploads
-- Called for each row in the CSV file
-- Parameters: $1 = row index, $2 = row data (text[]), $3 = previous result, $4 = metadata (json)
-- Metadata contains: type, fileName, contentType, size, success, status
...
begin
    ...
    if _index = 1 then
        truncate example_7.csv_uploads;
    end if;
