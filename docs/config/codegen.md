...
The `ReactQuery` subsection generates a TanStack Query v5 hooks module alongside the client. It is documented on its own page: [React Query Hooks](./react-query).
...
Available since version 3.20.0. The generator can emit a **TanStack Query v5 hooks module** alongside each generated client module — `useQuery` hooks for GET endpoints and `useMutation` hooks for everything else, configured under the `ClientCodeGen:ReactQuery` subsection shown in the [overview](#overview).

This feature has its own page: **[React Query Hooks](./react-query)**.
...
- [React Query Hooks](./react-query) - Generate TanStack Query v5 hooks alongside the TypeScript client
...
- [Dart Code Generation](./dart-codegen) - The Dart/Flutter counterpart of this generator
