---
description: "NpgsqlRest v3.16.0: Minor release fixing a long-standing class of bugs in the JSON-to-parameter parsers for the timestamp, timestamptz, time, and timetz…"
---
...
The existing `MultiParamsTests2` / `MultiParamsQueryStringTests2` test pairs already hinted at this — both used `Should().Match(t => t == "12:06:59..." || t == "11:06:59...")` style assertions with a comment that read *"integration server seems to have a different datetime altogether"*. That was the bug, papered over. After this fix both tests assert single deterministic values.
